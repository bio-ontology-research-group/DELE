...
class OntologyDataLoader:
    ...
    :type device: str
    :param go_classes: GO class indices
    :type go_classes: numpy.array(numpy.int64)
    :param protein_classes: protein class indices
    :type protein_classes: numpy.array(numpy.int64)
    ...
    def __init__(
        ...
        self.protein_classes = protein_classes
    ...
    def generate_negatives(self, gci_name, pos_batch):
        ...
        if gci_name == "gci0":
            ...
            go_mask = th.isin(data, self.go_classes)
            prot_mask = th.isin(data, self.protein_classes)
            ...
            prot_corrupted = np.random.choice(
                self.protein_classes.cpu().numpy(), size=len(prot_ixs), replace=True
            )
            prot_part[prot_ixs] = th.tensor(prot_corrupted).to(self.device)
            go_ixs = go_mask.nonzero().squeeze(1)
            go_corrupted = np.random.choice(
                self.go_classes.cpu().numpy(), size=len(go_ixs), replace=True
            )
            ...
            data = pos_batch[:, 1]
            go_mask = th.isin(data, self.go_classes)
            prot_mask = th.isin(data, self.prot_classes)
            go_part = data * go_mask
            prot_part = data * prot_mask
            go_ixs = go_mask.nonzero().squeeze(1)
            prot_ixs = prot_mask.nonzero().squeeze(1)
            prot_corrupted = np.random.choice(
                self.prot_classes.cpu().numpy(), size=len(prot_ixs), replace=True
            )
            prot_part[prot_ixs] = th.tensor(prot_corrupted).to(self.device)
            go_ixs = go_mask.nonzero().squeeze(1)
            go_corrupted = np.random.choice(
